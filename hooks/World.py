...
def after_create_regions(world: World, multiworld: MultiWorld, player: int):
    ...
    nf_low, nf_high = 0, 0
    if no_fail != "off":
        nf_low, nf_high = ParseTier(no_fail)
    count = 0
    for region in multiworld.regions:
        if region.player == player:
            for location in list(region.locations):
                T = location.parent_region
                T = T and T.name[-1]
                if T and T.isdigit() and (int(T) < low or int(T) > high):
                    region.locations.remove(location)
                    count += 1
                elif not world.options.enable_meme_maps.value and location.name.split(" - ")[0] in Meme:
                    region.locations.remove(location)
                    count += 1
                elif not world.options.enable_tedious_maps.value and location.name.split(" - ")[0] in Tedious:
                    region.locations.remove(location)
                    count += 1
                elif location.name.find("(No-Fail)") > -1:
...
def before_create_items_all(item_config: dict[str, int|dict], world: World, multiworld: MultiWorld, player: int) -> dict[str, int|dict]:
    ...
    for t in range(len(Tiers)):
        ...
        if T < low or T > high:
            for Map in Tiers[t]:
                assert item_config[Map] != 0, "oops doubling"
                item_config[Map] = 0
                count += 1

    if not world.options.enable_meme_maps.value:
        for Map in Meme:
            if item_config[Map] != 0:
                item_config[Map] = 0
                count += 1

    if not world.options.enable_tedious_maps.value:
        for Map in Tedious:
            if item_config[Map] != 0:
                item_config[Map] = 0
                count += 1
...
def before_create_items_starting(item_pool: list, world: World, multiworld: MultiWorld, player: int) -> list:
    ...
    if world.options.progressive_tiers.value:
        ...
        for item in item_table:
            if "region" in item and item["region"] == f"Tier {low}":
                potential_starts += 1
        # Direct indexes :)
        starting_items[0]["random"] = low - 1
        item_count = min(world.options.starting_maps.value, potential_starts)
